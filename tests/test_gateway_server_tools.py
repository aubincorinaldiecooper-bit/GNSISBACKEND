...
class ServerToolInjectionUnitTests(unittest.TestCase):
    ...
    def test_primary_call_appends_web_search_and_advisor(self):
        ...
        # Web Search uses OpenRouter's documented nested parameters shape.
        ws = payload["tools"][1]
        self.assertEqual(
            ws,
            {
        ...
        self.assertNotIn("engine", ws)
        self.assertNotIn("max_results", ws)
        self.assertNotIn("max_total_results", ws)
        self.assertNotIn("search_context_size", ws.get("parameters", {}))

        # Advisor is pinned under parameters.model with the expected config.
        advisor = payload["tools"][2]
        self.assertEqual(advisor["type"], "openrouter:advisor")
        self.assertNotIn("model", advisor)
        params = advisor["parameters"]
        self.assertEqual(params["name"], "advisor")
        self.assertEqual(params["model"], ALLOWED_ADVISOR)
        self.assertEqual(params["forward_transcript"], False)
        self.assertEqual(params["max_tool_calls"], 4)
...
class GatewayEndToEndTests(unittest.TestCase):
    ...
    def test_primary_call_ships_web_search_and_advisor_to_upstream(self):
        ...
        advisor = next(t for t in payload["tools"] if t["type"] == "openrouter:advisor")
        self.assertEqual(advisor["parameters"]["model"], ALLOWED_ADVISOR)

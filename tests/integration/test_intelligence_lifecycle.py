...
class IntelligenceLifecycleIntegrationTests(unittest.TestCase):
    ...
    def test_execution_run_normalizes_memory_ids_before_consumption_audit(self):
        from gnsis.service import orm
        from gnsis.service.codememory import CodeMemory, MemoryKind
        from gnsis.service.db import session_scope

        job = make_job(instruction="fix authentication helper")
        ...
        with session_scope() as s:
            s.add(
                orm.AgentMemory(
                    ...
                    content="authentication legacy helper one",
                    meta={},
                    approved=True,
                    workspace_id=job.workspace_id,
                    repository_id=job.repository_id,
            ...
            s.add(
                orm.AgentMemory(
                    repo=job.repo,
                    kind=MemoryKind.CONVENTION,
                    content="authentication legacy helper two",
                    meta={},
                    approved=True,
                    workspace_id=job.workspace_id,
                    repository_id=job.repository_id,
                    memory_id=None,
                    source_job_id="legacy-2",
                )
            )
        ...
        self.assertGreaterEqual(selection.memory_ids.count(""), 2)

        run_store, run = make_run(
            job,
            ["", "mem_A", " ", "mem_B", "mem_A", *selection.memory_ids, "mem_C", "mem_B"],
        )

        self.assertEqual(run.memory_ids, ["mem_A", "mem_B", "mem_C"])
        self.assertEqual(run_store.get_run(run.id).memory_ids, ["mem_A", "mem_B", "mem_C"])
        with session_scope() as s:
            rows = (
                s.query(orm.MemoryConsumption)
                .filter(orm.MemoryConsumption.run_id == run.id)
                .order_by(orm.MemoryConsumption.id)
                .all()
            )
            self.assertEqual([row.memory_id for row in rows], ["mem_A", "mem_B", "mem_C"])
            self.assertEqual(
                s.query(orm.MemoryConsumption)
                .filter(
                    orm.MemoryConsumption.run_id == run.id,
                    orm.MemoryConsumption.memory_id == "",
                )
                .count(),
                0,
            )

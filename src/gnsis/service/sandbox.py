...
_INSTRUCTION = ".gnsis-instruction.txt"
_EVENTS = ".gnsis-events.jsonl"
_RESULT = ".gnsis-result.json"
...
class DockerEngine:
    ...
    def generate(
        ...
        self._write(workspace.path, _INSTRUCTION, instruction)
        cmd = self._docker_command(workspace)
        ...
        self._replay_events(workspace.path, sink)
        result = self._read_result(workspace.path)
        self._cleanup(workspace.path)
    ...
    # -- helpers ----------------------------------------------------------
    def _docker_command(self, workspace: Workspace) -> List[str]:
        cmd = [
            "docker", "run", "--rm",
        ...
        cmd += [
            ...
            "--instruction-file", f"/work/{_INSTRUCTION}",
            "--events", f"/work/{_EVENTS}",
            "--result", f"/work/{_RESULT}",
        ]
        return cmd

    def _replay_events(self, path: str, sink: PhaseSink) -> None:
        events_path = os.path.join(path, _EVENTS)
    ...
    def _read_result(self, path: str) -> Optional[dict]:
        result_path = os.path.join(path, _RESULT)
    ...
    def _cleanup(self, path: str) -> None:
        for name in (_INSTRUCTION, _EVENTS, _RESULT):
            try:

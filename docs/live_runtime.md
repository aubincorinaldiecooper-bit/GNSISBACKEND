...
3. Compare the settings with the app being replaced: `GANDER_HEALTH_URL=https://<the old app's address> python scripts/verify-modal-gander.py` prints the same fields for it, with no Modal token. CLIPIT #134 said the live app's duplex settings differed from the checked-in configuration in nine values, of which `/health` shows five (`chunk_ms`, `sliding_window_mode`, `context_max_units`, `context_previous_max_tokens`, `task_slate_visible_to_model`). If the old app reports different values, put them into `gander/configs/gnsis-live.yaml` and redeploy before switching; the other four (`trailing_silence_sec`, `turn_bind_grace_sec`, `speak_text_tokens_per_unit`, `max_new_speak_tokens_per_chunk`, and the two tool token limits) are only visible in the old app's own configuration.

...
## Sandboxing (executing model-written code) — retired

User jobs execute exclusively through the GitHub Actions executor (a separate,
hardened, SHA-pinned repository) — see its own docs for that isolation model.

`GNSIS_SANDBOX=docker` and `gnsis.service.sandbox.DockerEngine` (a self-hosted
alternative that ran a job's engine inside a container the worker itself
launched) predate the GitHub Actions executor and are no longer wired into the
user-job path. `Settings.sandbox` and its related fields are retained only as a
design reference for the hardened container command and for isolated tests —
setting `GNSIS_SANDBOX` no longer changes runtime behavior.

## Railway public-beta services

Production runs three separate Railway services from `aubincorinaldiecooper-bit/GNSISBACKEND`:
...
```sh
celery -A gnsis.service.tasks.celery_app beat --loglevel=info --pidfile=/tmp/celerybeat.pid --schedule=/tmp/celerybeat-schedule
```

Replicas: **exactly 1**. Do not run more than one beat replica and do not add `-B` to the worker command; duplicate beat schedulers can enqueue duplicate reconciliation/CI-observation work.

Required variables for `GNSISBEAT`:

- `GNSIS_SERVICE_ROLE=beat`
- `DATABASE_URL`
- `REDIS_URL`
- `CELERY_BROKER_URL` (if different from `REDIS_URL`)
- `CELERY_RESULT_BACKEND` (if different from `REDIS_URL`)

`GNSISBEAT` must not receive API-only secrets such as `OPENROUTER_API_KEY`, `GITHUB_WEBHOOK_SECRET`, Better Auth secrets, or `GNSIS_AUTH_INTERNAL_SECRET`.

### GNSISBACKEND variables
...
```sh
POST /internal/compute/gnsis/status?smoke=false
POST /internal/compute/gnsis/deploy

...
def _reserve_built_in_tools(params: Any, bundle: Any) -> Any:
    ...
    (Codex's finding on a prior implementation review). The budget is raised here by exactly what
...
def _split_model_haptics(event: Any) -> _HapticSplit:
    ...
    call was thrown away with it (Codex's finding on a prior implementation review). So the
    ...
    *,
    ...
    async def live_qr(request: Request) -> Response:
        ...
        try:
            ...
            return Response(
                "QR rendering needs segno: install gnsis-runtime[qr]",
                status_code=501,
